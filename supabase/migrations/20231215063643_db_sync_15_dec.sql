...
CREATE
OR REPLACE FUNCTION public.applications_inview(
  job_id uuid,
  min_lat double precision,
  min_long double precision,
  max_lat double precision,
  max_long double precision
) RETURNS jsonb [] LANGUAGE plpgsql AS $ function $ DECLARE result JSONB [];
...
FROM
  (
    SELECT
      ...
      st_x(geolocation :: geometry) as long
    FROM
      public.job_applications
    WHERE
...
CREATE
OR REPLACE FUNCTION public.batchsavegreenhouse_test(rec_id uuid) RETURNS jsonb [] LANGUAGE plpgsql AS $ function $ DECLARE result JSONB [];
...
CREATE
OR REPLACE FUNCTION public.calc_sim_score2(
  job_ids uuid [],
  skill_qry_emb vector,
  edu_qry_emb vector,
  exp_qry_emb vector,
  resume_qry_emb vector,
  max_records integer DEFAULT 25,
  ts_query text DEFAULT '' :: text,
  filter_companies text DEFAULT '' :: text
) RETURNS TABLE(
  application_id uuid,
  created_at text,
  first_name citext,
  ...
  sim_exp double precision,
  sim_res double precision,
  sim_skills double precision,
  sim_educ double precision
) LANGUAGE plpgsql AS $ function $ BEGIN RETURN QUERY
SELECT
  DISTINCT ON (ja.candidate_id) ja.application_id,
  ja.created_at :: text,
  c.first_name,
  c.last_name,
  ...
  (
    (
      COALESCE(1 - (ja.experience_embedding <= > exp_qry_emb), 0) * 0.5 + COALESCE(1 - (ja.resume_embedding <= > resume_qry_emb), 0) * 0.2 + COALESCE(1 - (ja.skills_embedding <= > skill_qry_emb), 0) * 0.2 + COALESCE(1 - (ja.education_embedding <= > edu_qry_emb), 0) * 0.1
    )
  ) AS similarity,
...
WHERE
  ...
  AND CASE
    WHEN LENGTH(ts_query) > 0 THEN to_tsvector(
      COALESCE(
        lower(ja.json_resume -> 'basics' ->> 'currentJobTitle'),
        ''
      )
...
CREATE
OR REPLACE FUNCTION public.job_application_filter_sort(
  jb_id uuid,
  min_lat double precision DEFAULT 0,
  min_long double precision DEFAULT 0,
  max_lat double precision DEFAULT 0,
  max_long double precision DEFAULT 0,
  j_status text DEFAULT 'new' :: text,
  from_rec_num numeric DEFAULT 0,
  end_rec_num numeric DEFAULT 100,
  min_resume_score numeric DEFAULT 0,
  max_resume_score numeric DEFAULT 100,
  min_interview_score numeric DEFAULT 0,
  max_interview_score numeric DEFAULT 100,
  sort_column_text text DEFAULT 'resume_score' :: text,
  is_sort_desc boolean DEFAULT true,
  text_search_qry text DEFAULT '' :: text,
  is_locat_filter_on boolean DEFAULT false
) RETURNS TABLE(job_app json, cand json, total_results bigint) LANGUAGE plpgsql AS $ function $ BEGIN -- Initialize total_results variable
total_results := 0;

-- Return the paginated results along with total_results
RETURN QUERY WITH filtered_results AS (
  SELECT
    json_build_object(
      'application_id',
      ja.application_id,
  ...
  WHERE
    ...
    AND ja.resume_score >= min_resume_score
    AND ja.resume_score <= max_resume_score
    AND ja.interview_score >= min_interview_score
    AND ja.interview_score <= max_interview_score
    AND (
...
ORDER BY
  CASE
    WHEN sort_column_text = 'resume_score'
    AND is_sort_desc THEN fr.job_app ->> 'resume_score'
  END DESC,
  CASE
    WHEN sort_column_text = 'resume_score'
    AND NOT is_sort_desc THEN fr.job_app ->> 'resume_score'
  END ASC,
  CASE
    WHEN sort_column_text = 'full_name'
    AND is_sort_desc THEN concat(fr.cand ->> 'first_name', ' ', fr.cand ->> 'last_name')
  END DESC,
  CASE
    WHEN sort_column_text = 'full_name'
    AND NOT is_sort_desc THEN concat(fr.cand ->> 'first_name', ' ', fr.cand ->> 'last_name')
  END ASC,
  CASE
    WHEN sort_column_text = 'email'
    AND is_sort_desc THEN fr.cand ->> 'email'
  END DESC,
  CASE
    WHEN sort_column_text = 'email'
    AND NOT is_sort_desc THEN fr.cand ->> 'email'
  END ASC,
  CASE
    WHEN sort_column_text = 'created_at'
    AND is_sort_desc THEN fr.job_app ->> 'created_at'
  END DESC,
  CASE
    WHEN sort_column_text = 'created_at'
    AND NOT is_sort_desc THEN fr.job_app ->> 'created_at'
  END ASC,
  CASE
    WHEN sort_column_text = 'interview_score'
    AND is_sort_desc THEN fr.job_app ->> 'interview_score'
  END DESC,
  CASE
    WHEN sort_column_text = 'interview_score'
    AND NOT is_sort_desc THEN fr.job_app ->> 'interview_score'
  END ASC
LIMIT
  (end_rec_num - from_rec_num) OFFSET from_rec_num;
...
CREATE
OR REPLACE FUNCTION public.ashbyjobreference(rec_id uuid) RETURNS jsonb [] LANGUAGE plpgsql AS $ function $ DECLARE result JSONB [];
...
CREATE
OR REPLACE FUNCTION public.batchcalcresumejdscore() RETURNS jsonb [] LANGUAGE plpgsql AS $ function $ DECLARE result JSONB [];
...
FROM
  (
    ...
    WHERE
      ja.api_status = 'not started'
      and ja.resume is not null
...
CREATE
OR REPLACE FUNCTION public.levercandidatesync() RETURNS jsonb LANGUAGE plpgsql AS $ function $ DECLARE result JSONB;
...
BEGIN -- Initialize an empty JSON array for the results
result := '[]' :: JSONB;
