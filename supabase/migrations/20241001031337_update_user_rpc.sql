...
OR REPLACE FUNCTION public.update_user(
    ...
    role_id uuid,
    recruiter_id uuid
) RETURNS void LANGUAGE plpgsql SECURITY DEFINER AS $ function $ BEGIN -- Update recruiter_user table
UPDATE
    recruiter_user
SET
    department_id = update_user.department_id,
    -- Use the function parameter explicitly
