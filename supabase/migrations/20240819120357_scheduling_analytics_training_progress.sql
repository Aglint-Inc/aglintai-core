...
CREATE OR REPLACE FUNCTION public.scheduling_analytics_training_progress(recruiter_id uuid, jobs uuid[] DEFAULT NULL::uuid[])
    ...
    with
      ...
      filtered_module_members as (
        ...
        where
          ...
          and modules.job_id <> ALL (COALESCE(scheduling_analytics_training_progress.jobs, ARRAY[]::uuid[]))
          and interview_module_relation.training_status = 'training'
      )
    select
      *
    from
