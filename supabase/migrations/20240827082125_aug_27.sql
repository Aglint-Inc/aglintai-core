...
CREATE
OR REPLACE FUNCTION get_all_interviewers (
  recruiter_id_param UUID,
  start_time_param TIMESTAMP WITH TIME ZONE,
  end_time_param TIMESTAMP WITH TIME ZONE,
  department_ids_params INTEGER[], -- Array of department IDs
  office_location_ids_params INTEGER[],
  job_ids_params UUID[],
  module_ids_params UUID[]-- Array of job IDs
) RETURNS TABLE (
  user_id UUID,
  first_name TEXT,
  last_name TEXT,
  ...
  upcoming_meeting_count bigint,
  completed_meeting_count bigint,
  completed_meetings JSONB
) LANGUAGE plpgsql AS $$
BEGIN
    RETURN QUERY
    SELECT 
