...
create or replace view "public"."application_view" as  WITH application_candidate_cte AS (
         ...
         SELECT interview_meeting.application_id,
            ...
            interview_plan.name,
            json_build_object('not_scheduled', count(interview_meeting.status) FILTER (WHERE (interview_meeting.status = 'not_scheduled'::interview_schedule_status)), 'confirmed', count(interview_meeting.status) FILTER (WHERE (interview_meeting.status = 'confirmed'::interview_schedule_status)), 'completed', count(interview_meeting.status) FILTER (WHERE (interview_meeting.status = 'completed'::interview_schedule_status)), 'waiting', count(interview_meeting.status) FILTER (WHERE (interview_meeting.status = 'waiting'::interview_schedule_status)), 'cancelled', count(interview_meeting.status) FILTER (WHERE (interview_meeting.status = 'cancelled'::interview_schedule_status)), 'reschedule', count(interview_meeting.status) FILTER (WHERE (interview_meeting.status = 'reschedule'::interview_schedule_status))) AS status
          ...
          GROUP BY interview_meeting.application_id, interview_plan.id, interview_plan.plan_order, interview_plan.name
