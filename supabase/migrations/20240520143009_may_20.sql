...
alter table "public"."applications" add column "bookmarked" boolean not null default false;

alter table "public"."applications" add column "is_new" boolean not null default false;

alter table "public"."interview_meeting" add column "organizer_id" uuid;

alter table "public"."interview_session_cancel" add column "cancel_user_id" uuid;
...
alter table "public"."interview_session_cancel" add constraint "interview_session_cancel_cancel_user_id_fkey" FOREIGN KEY (cancel_user_id) REFERENCES recruiter_user(user_id) ON DELETE SET NULL not valid;
...
BEGIN
    ...
    WHERE pj.recruiter_id = target_rec_id AND intmeet.status IN ('completed', 'confirmed', 'cancelled','waiting') 
