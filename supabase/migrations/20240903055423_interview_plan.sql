revoke delete on table "public"."candidate_portal_plan" from "anon";

revoke insert on table "public"."candidate_portal_plan" from "anon";

revoke references on table "public"."candidate_portal_plan" from "anon";

revoke select on table "public"."candidate_portal_plan" from "anon";

revoke trigger on table "public"."candidate_portal_plan" from "anon";

revoke truncate on table "public"."candidate_portal_plan" from "anon";

revoke update on table "public"."candidate_portal_plan" from "anon";

revoke delete on table "public"."candidate_portal_plan" from "authenticated";

revoke insert on table "public"."candidate_portal_plan" from "authenticated";

revoke references on table "public"."candidate_portal_plan" from "authenticated";

revoke select on table "public"."candidate_portal_plan" from "authenticated";

revoke trigger on table "public"."candidate_portal_plan" from "authenticated";

revoke truncate on table "public"."candidate_portal_plan" from "authenticated";

revoke update on table "public"."candidate_portal_plan" from "authenticated";

revoke delete on table "public"."candidate_portal_plan" from "service_role";

revoke insert on table "public"."candidate_portal_plan" from "service_role";

revoke references on table "public"."candidate_portal_plan" from "service_role";

revoke select on table "public"."candidate_portal_plan" from "service_role";

revoke trigger on table "public"."candidate_portal_plan" from "service_role";

revoke truncate on table "public"."candidate_portal_plan" from "service_role";

revoke update on table "public"."candidate_portal_plan" from "service_role";

alter table "public"."candidate_portal_plan" drop constraint "candidate_interview_plan_application_id_fkey";
...
alter table "public"."candidate_portal_plan" drop constraint "candidate_interview_plan_pkey";

drop index if exists "public"."candidate_interview_plan_pkey";

drop table "public"."candidate_portal_plan";
...
alter table "public"."candidate_portal_job" disable row level security;

alter table "public"."candidate_portal_message" disable row level security;

alter table "public"."interview_meeting" drop column "schedule_request_id";
...
alter table "public"."interview_progress" add constraint "interview_progress_application_id_fkey" FOREIGN KEY (application_id) REFERENCES applications(id) not valid;

alter table "public"."interview_progress" validate constraint "interview_progress_application_id_fkey";

alter table "public"."interview_progress" add constraint "interview_progress_job_id_fkey" FOREIGN KEY (job_id) REFERENCES public_jobs(id) not valid;
...
BEGIN
  IF NEW.status = 'interview' THEN
    INSERT INTO "candidate_portal_job" (job_id,application_id,images,greetings,banner) 
   VALUES (NEW.job_id,NEW.id,'{"https://stat.overdrive.in/wp-content/odgallery/2022/07/63248_2022_Suzuki_Brezza_1.jpg",
   "https://img.freepik.com/free-photo/abstract-autumn-beauty-multi-colored-leaf-vein-pattern-generated-by-ai_188544-9871.jpg"}','Welcome to our portal! We’re here to keep you connected every step of the way, whether you’re preparing for interviews or exploring new opportunities. Dive into detailed company profiles, discover comprehensive job descriptions, and don"t hesitate to reach out with any questions you may have. We"re committed to helping you find the right fit and making your job search experience as smooth as possible.','https://t4.ftcdn.net/jpg/04/67/93/01/360_F_467930159_UcfrOkjhFG436zoT9fSetYccBgpNkokp.jpg' );
...
CREATE TRIGGER candidate_move_new_to_interview AFTER UPDATE OF status ON public.applications FOR EACH ROW WHEN (((old.status = 'new'::application_status) AND (new.status = 'interview'::application_status))) EXECUTE FUNCTION trigger_candidate_portal();
