...
function processDirectory(rootDirs, outputFiles) {
  ...
  outputFiles.forEach(function (item) {
    var _a;
    var tempResult =
      ((_a = item.filter) === null || _a === void 0
        ? void 0
        : _a.call(item, item.filter(result))) || result;
    fs.writeFileSync(
      item.path,
      'export const '.concat(item.objectName, ' = [\n').concat(
        tempResult
          .map(function (item) {
            return "'" + item + "'";
          })
