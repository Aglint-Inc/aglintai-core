...
interface EmptyStateProps {
  module: "jobs" | "interviewPlan" | "applications" | "candidates" | "interviewReports" | "dashboard"
  title: string
...
export function EmptyState({ module, title, description, actionLabel, onAction }: EmptyStateProps) {
    ...
    <Card className="w-full max-w-md mx-auto border-none shadow-none">
      <CardContent className="pt-6 px-6 pb-8 flex flex-col items-center text-center">
        <div className="flex items-center justify-center mb-4">
          <Icon className="w-16 h-16 text-primary" strokeWidth={.5} size={48}/>
        </div>
        <h3 className="text-lg font-semibold mb-2">{title}</h3>
        {description && <p className="text-muted-foreground mb-6">{description}</p>} {/* Conditionally render description */}
