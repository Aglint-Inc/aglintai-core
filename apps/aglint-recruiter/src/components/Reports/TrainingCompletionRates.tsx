...
import { DownloadIcon, ArrowUpDown, CalendarIcon, Search } from "lucide-react"
...
export default function Component() {
  ...
  const filteredData = reportData.filter(row => {
    ...
    const dateMatch = (!startDate || new Date(row.dateEntered) >= startDate) &&
            ...
            <div className="flex items-center space-x-2">
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="outline" className="w-[130px] justify-start text-left font-normal">
                    ...
                    {startDate ? format(startDate, "MM/dd/yy") : <span>Start Date</span>}
                  </Button>
                </PopoverTrigger>
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="outline" className="w-[130px] justify-start text-left font-normal">
                    <CalendarIcon className="mr-2 h-4 w-4" />
                    {endDate ? format(endDate, "MM/dd/yy") : <span>End Date</span>}
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-auto p-0">
                  <Calendar
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-b border-border">
                ...
                <TableHead>
                  <Button variant="ghost" onClick={() => handleSort('dateEntered')}>
                    Date entered <ArrowUpDown className="ml-2 h-4 w-4" />
                  </Button>
                </TableHead>
                <TableHead>
                  <Button variant="ghost" onClick={() => handleSort('daysInTraining')}>
                    Days in training <ArrowUpDown className="ml-2 h-4 w-4" />
                  </Button>
                </TableHead>
                <TableHead>
                  <Button variant="ghost" onClick={() => handleSort('graduationDate')}>
                    Graduation date <ArrowUpDown className="ml-2 h-4 w-4" />
                  </Button>
                </TableHead>
                <TableHead>
                  <Button variant="ghost" onClick={() => handleSort('averageTimeToGraduation')}>
                    Avg. time to graduation <ArrowUpDown className="ml-2 h-4 w-4" />
                  </Button>
                </TableHead>
              </TableRow>
            </TableHeader>
