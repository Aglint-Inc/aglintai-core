...
export default function CandidateDropoutFunnelReport() {
  ...
  const stageCounts = useMemo(() => {
    return stages.map(stage => ({
      stage,
      count: candidateData.filter(candidate => candidate.stage === stage).length
    }))
  }, [])
      ...
      <CardContent>
        <div className="mb-8">
          <div className="flex justify-between items-end h-64">
            {stageCounts.map((stageData, index) => (
              <div key={stageData.stage} className="flex flex-col items-center w-1/4">
                <div 
                ...
                <div className="text-xs text-center mt-2">{stageData.stage}</div>
              </div>
            ))}
          </div>
        </div>
