...
export const schedulingAnalyticsRouter = createTRPCRouter({
  filters: privateProcedure
    ...
    .query(async ({ input: { recruiter_id } }) => {
  ...
  completed_interviews: privateProcedure
    ...
    .query(async ({ input: { recruiter_id, departments, jobs, type } }) => {
      const db = createPrivateClient();
      return (
        await db
  ...
  decline_requests: privateProcedure
    ...
    .query(async ({ input: { recruiter_id, departments, jobs } }) => {
      const db = createPrivateClient();
      return (
        await db
  ...
  training_progress: privateProcedure
    ...
    .query(
      async ({ input: { recruiter_id, departments, jobs, locations } }) => {
        const db = createPrivateClient();
        return (
          await db
