...
export const recentDecline = privateProcedure.query(
  async ({ ctx: { recruiter_id } }) => {
    const adminDb = createPublicClient();
    return getRecentDecline(adminDb, recruiter_id);
  },
);

export type RecentDeclineType = ProcedureDefinition<typeof recentDecline>;

async function getRecentDecline(
  ...
  const { data, error } = await supabase
    ...
    .eq('is_resolved', true)
    .neq('type', 'candidate_request_reschedule')
