...
export const recentReschedule = privateProcedure.query(
  async ({ ctx: { recruiter_id } }) => {
    const adminDb = createPublicClient();
    return getRecentReschedule(adminDb, recruiter_id);
  },
);

export type RecentRescheduleType = ProcedureDefinition<typeof recentReschedule>;

async function getRecentReschedule(
