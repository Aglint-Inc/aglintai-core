...
const query = async ({
  ...
  const { data: dataModule } = await db
    ...
    .select(
      '*,departments(*),interview_module_approve_users(*),interview_module_relation!inner(*,all_interviewers!inner(user_id,first_name,last_name,scheduling_settings,total_hours_this_week,total_interviews_this_week,total_hours_today,total_interviews_today, profile_image,position,email))',
    )
    .eq('id', module_id)
    .throwOnError()
    .single();
