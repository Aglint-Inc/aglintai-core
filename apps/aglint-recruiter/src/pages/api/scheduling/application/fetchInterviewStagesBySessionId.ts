  ...
  try {
    ...
    requestHandler(
      ...
      async ({ body }) => {
        ...
        const sessions = await fetchDetails(application_id, sessions_ids);
        return {
          success: true,
          sessions,
        };
...
const fetchSessionDetails = async ({
  ...
  const { data } = await supabaseCaller
    ...
    .select(
      `*,interview_meeting(*),${interviewCancelReasons},interview_module(*),interview_session_relation(*,interview_module_relation(*,${userDetails}),debrief_user:${userDetails})`,
    )
    .in('id', session_ids);

  if (!data) return [];

  const reducedPlan = data.map((ses) => {
    return {
