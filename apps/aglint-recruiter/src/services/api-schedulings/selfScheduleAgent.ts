...
export const selfScheduleAgent = async ({
  ...
  const [filter_json] = supabaseWrap(
    await supabaseAdmin
      ...
      .select(
        '*,applications(public_jobs(id,job_title,recruiter!public_jobs_recruiter_id_fkey(id,name)),candidates(*))',
      ),
  );
  ...
  if (agent_type === 'phone') {
    const job_details = filter_json.applications.public_jobs;
    const candidate = filter_json.applications.candidates;
    const bodyParams = {
