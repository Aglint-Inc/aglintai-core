...
test.describe('Candidate reschedule request', () => {
  const REQUEST_INDEX = 2; //request inde
  test('Request reschedule', async ({
    loginPage,
    ...
    await test.step('Navigate availability page details', async () => {
      ...
      expect(bookedAvailability.length).toBeGreaterThanOrEqual(
        REQUEST_INDEX + 1,
      );
      availability_id = bookedAvailability[REQUEST_INDEX].id;
