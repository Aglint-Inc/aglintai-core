...
function convertJsonToText(resume_json: schemaType) {
  const work =
  ...
  const skills = Array.isArray(resume_json.skills)
    ? resume_json.skills.join(' ')
    : '';

  const certificates = Array.isArray(resume_json.certificates)
  ...
  const languages = Array.isArray(resume_json.languages)
...
export const getEmbeddings = async (openai: OpenAI, resume_json: any) => {
  const resumeSections = convertJsonToText(resume_json as schemaType);
  const embeddings: {
