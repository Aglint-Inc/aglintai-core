...
export const agentPrompt = (cand_phone: string) => {
  ...
  const interviewSchedule =
    cand_info +
    `# Instructions to scheduling the interview:\n` +
  ...
  const end_call = `* after sucessfully completing each candidate's request ask whether he need any assistance or to end the call.\n`;
  ...
  const getAgentPrompt = () => {
    ...
    if (
      ...
      let agent_task = '';
      agent_task = `Your job is to help the candidate to assist further or to end the call :\n\n`;
      all_prompt =
        agent_role +
        agent_task +
        interview_dates +
