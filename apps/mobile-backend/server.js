...
const callOpenAIWithTool = async (input) => {
    ...
    try {
        ...
        const response = await openai.chat.completions.create({
            ...
            messages: [
                { role: "system", content: "You are an assistant that helps schedule interviews." },
                { role: "user", content: input }
            ],
            functions: [
                {
                    "name": "extract_details",
                    "description": "Extract job, candidate name, date range, and interview name",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "job": { "type": "string" },
...
app.post('/get-availability', verifyToken, async (req, res) => {
    ...
    if (!inputString || typeof inputString !== 'string') {
        res.write('data: Error: Invalid input: input should be a non-empty string\n\n');
        res.end();
        return;
    }
